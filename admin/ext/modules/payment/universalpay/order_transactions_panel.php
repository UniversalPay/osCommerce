...
if (isset($order_id) && tep_not_null($order_id) && isset($_GET['action']) && $_GET['action']=='edit') {
    ...
    $query = tep_db_query("SELECT * FROM `universalpay_order` WHERE `order_id`='" . $order_id . "'");
    $order = tep_db_fetch_array($query);
//     var_dump($order);
    if($order){
        $transactions_query = tep_db_query("SELECT * FROM `universalpay_transactions` WHERE `universalpay_order_id`='" . $order['universalpay_order_id'] . "'");
        $total = $order['total'];
        $refund_query = tep_db_query("SELECT SUM(`amount`) AS `refund_total` FROM `universalpay_transactions` WHERE `universalpay_order_id` = '" . $order['universalpay_order_id'] . "' AND `type` = 'refund'");
        ...
        echo '<h1>UniversalPay Payment</h1>';
        echo '<table border="1px" width="50%" cellspacing="0" cellpadding="15">';
        
        echo    '<tr>';
        echo        '<td class="smallText" align="center"><strong>UniversalPay ID</strong></td>';
    ...
    var action_url = "ext/modules/payment/universalpay/actions.php";
